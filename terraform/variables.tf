...
variable "instance_type" {
  type        = string
  description = "EC2 instance type. t3.medium gives 2 vCPU / 4 GB, sufficient for k3s + Minecraft"
  default     = "t3.medium"
}
...
variable "ecr_image_uri" {
  type        = string
  description = "Full ECR URI with pinned tag, e.g. 123456789.dkr.ecr.us-east-1.amazonaws.com/minecraft-server:mc-1.21.4-build1"
}
...
variable "student_id" {
  type        = string
  description = "Shown in the server MOTD to satisfy the rubric requirement"
  default     = "henderos"
}
